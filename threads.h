...
struct SCriticalSectionImplStategy
{
   ...
   //Блокировка на чтение/запись не разделяется, реализуется через Lock/Unlock
   static void LockRead( TLockType &LT ) { Lock(LT); }
   static void UnLockRead( TLockType &LT ) { UnLock(LT); }
   static void LockWrite( TLockType &LT ) { Lock(LT); }
   static void UnLockWrite( TLockType &LT ) { UnLock(LT); }
};
...
struct SSWMRImplStategy
{
   ...
   //Обычные методы Lock/Unlock реализуют блокировку на запись
   static void Lock( TLockType &LT ){ LockWrite(LT); }
   static void UnLock( TLockType &LT ){ UnLockWrite(LT); }
};
