...
///////////////////////////////////////////////////////////////////////////////
// Переработка контейнера Loki:
// Добавление функционала, контейнера MultiAssocVector и испраления ошибок
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
   ...
   template
      ...
      public:
         ...
         {
            //NW 205-05-08 
            //Здесь Alexandrescu был не прав, фактически у него проверяется 
            //условие  val > *pos и val > *(pos + 1) и val >= *(pos + 1)
            //if (pos != end() && this->operator()(*pos, val) && 
            //    (pos == end() - 1 ||
            //        !this->operator()(val, pos[1]) &&
            //            this->operator()(pos[1], val)))
            //т.е. проходит следующий тест: 
            ...
            //На самом деле элемент должно быть *(pos - 1) < val < *pos 
            if(
         ...
         //NW 05.05.2005
         //Позволяет изменить ключ элемента при этом не удаляя, 
         //а затем вставляя элемент с новымм ключём
         //Работает эффективней чем удаление и последующая вставка 
         //Возвращает: first: итератор элемента с ключом key 
         //            second: true - удалось изменить ключ, false - ключ key уже есть в контейнере.
         std::pair<iterator, bool> ChangeKey( iterator pos, const key_type& key )
         {
            //Если порядок элементов не изменился то не будем выполнять какие либор перемещения
            if(
            ...
            //Находим позицию нового элемента
            ...
            //else - Такой элемент уже есть, менять ничего не будем
      ...
      ////////////////////////////////////////////////////////////////////////////
      // Класс является подобием AssocVector, но в отличии от него позволяет 
      // хранить не уникальные ключи, так же как и multimap
      ////////////////////////////////////////////////////////////////////////////
      template
         <
            class K,
         ...
         public:
            ...
            //NW 05.05.2005
            //Позволяет изменить ключ элемента при этом не удаляя 
            //(и затем вставляя) элемент с новым ключём
            //При этом не гарантируется что элемент с новым ключём будет 
            //самым последнем из элементов с одинаковыми ключами (т.е. 
            //элемент пытается максимально долго сохранить свою позицию)
            //Работает эффективней чем удаление и последующая вставка 
            //Возвращает: итератор элемента с ключом key 
